...
// sem_init + error checking
#define SEM_DESTROY(name)                         \
	do {                                          \
		if (sem_destroy(&shm->sem_##name)) {      \
			perror("sem_init(" #name ") failed"); \
		}                                         \
	} while (0)
...
int main(int argc, char *argv[]) {
	...
	int elfID = 1;
	for (int i = 0; i < NE; i++, elfID++) {
		create_process(proc_elf, elfID);
	}
	for (int rdID = 1; rdID <= NR; rdID++) {
		create_process(proc_reindeer, rdID);
	}

	// if in bonus mode, wait for user signals
	while (bonus) {
		static int sig;
		...
		if (sig == SIGUSR1) {
			DPRINT("SIGUSR1");
			const int rand_NE = rand() % (NE + 1);
			for (int i = 0; i < rand_NE; i++, elfID++) {
				create_process(proc_elf, elfID);
			}
		} else {
			DPRINT("SIGUSR2");
...
void proc_santa(int arg) {
	...
	while (1) {
		...
		if (shm->reindeer == NR) {
			...
			SEM_POST(mutex);
			// notify main process to stop generating elves
			if (bonus) {
...
void create_process(void (*proc)(int), int arg) {
	...
	if (pid == 0) {
		// restore original quit sigaction
...
/**
 * @brief	custom handler for user signals
*/
...
void initialize() {
	...
	if (bonus) {
		// set signals for main process to wait for
		sigemptyset(&sigset);
		sigaddset(&sigset, SIGUSR1); // generate elves
		sigaddset(&sigset, SIGUSR2); // christmas started
		// register custom signal handler
		struct sigaction sa = {0};
		sa.sa_handler = &sig_handler;
		sigaction(SIGUSR1, &sa, NULL);
		sigaction(SIGUSR2, &sa, NULL);
		sigaction(SIGQUIT, &sa, &sa_sigquit);
	}
...
/**
 * @brief	properly destroys semaphores before shm deletion
*/
void destroy_sems() {
	SEM_DESTROY(mutex);
	SEM_DESTROY(sleigh_ready);
	SEM_DESTROY(hitch_reindeer);
	SEM_DESTROY(help);
	SEM_DESTROY(elf_queue);
	SEM_DESTROY(thanks_santa);
	SEM_DESTROY(wake_santa);
	DPRINT("sem destroyed");
}
...
void argparse(int argc, char *argv[]) {
	...
	int err = 0;
	err += str2i(argv[argc - 4], &NE);
	err += str2i(argv[argc - 3], &NR);
	err += str2i(argv[argc - 2], &TE);
	err += str2i(argv[argc - 1], &TR);
	if (err) {
